...
Copy-Item -Force -Path $root/src/exif-tool-dialog.js -Destination $buildDir > $null

Info "Insert the version=$version in the script"
(Get-Content $buildDir/exif-tool-dialog.js).Replace(
    "  data.version = `"0.0-dev`"",
    "  data.version = `"$version`"") | Set-Content $buildDir/exif-tool-dialog.js
